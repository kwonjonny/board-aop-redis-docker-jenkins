...
// List Notice Reply
const listReplyNotice = async (replyLast = false, page = 1) => {
    console.log("댓글 마지막 여부: ", replyLast, "페이지: ", page)
    const {data } = await axios.get(`${replyLink}notice/list/${nno}?page=${page}&replyLast=${replyLast}`)
    return data.result;
}

// Create Notice Reply 
const createReplyNotice = async (list) => {
    console.log("댓글 생성: ", list)
    const { data } = await axios.post(`${replyLink}notice/create`, list)
    return data.result;
}

// Update Notice Reply 
const updateReplyNotice = async (list) => {
    console.log("댓글 수정: " ,list)
    const { data } = await axios.put(`${replyLink}notice/update`, list)
    return data.result;
}

// Delete Notice Reply 
const deleteReplyNotice = async (nno) => {
    console.log("댓글 삭제: ", nno)
    const { data } = await axios.delete(`${replyLink}notice/delete/${nno}`)
    return data.result;
}

// Count Notice Reply 
const countReplyNotice = async (nno) => {
    console.log("댓글 수", nno)
    const { data } = await axios.get(`${replyLink}notice/count/${nno}`)
    return data.result;
}

// Get Notice User Details 
const getUserDetailsNotice = async () => {
    const { data } = await axios.get(`${replyLink}notice/user/email`)
...
const listReplyAxios = (replyLast, page) => {
    getUserDetailsNotice().then(currentUserEmail => {

    listReplyNotice(replyLast, page).then(arr => {
        let replyStr = ""
        let replyPagingStr = ""
        for(let i = 0; i< arr.list.length; i++) {
            const { reply, replyer, createDate, updateDate, step, gno, rno, isDeleted, nno} = arr.list[i]
            console.log("스탭", arr.list.map(item => item.step));
            ...
            <button class="btn btn-outline-secondary reply-update" data-reply="replyUpdate"  data-gno="${gno}" data-nno="${nno}" data-rno="${rno}" data-replyer="${replyer}" data-reply="${reply}">수정</button>
            ` : ''}
...
document.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (e.target && e.target.classList.contains("reply-replyChild")) {
            ...
            if (target.dataset.reply === "replyChild") {
                ...
                <div class="reply-form-wrapper">
                    ...
                    <textarea class="form-control reply-input" rows="3" placeholder="답글 내용을 입력하세요" data-gno="${gno}" data-nno="${nno}"></textarea>
                ...
                replySubmitBtn.addEventListener("click", () => {
                    ...
                    const nno = replyInput.dataset.nno;
                    const rno = target.dataset.rno;
                    ...
                    // createReplyNotice 함수 호출
                    createReplyNotice(list)
                        .then((response) => {
                            // 답글 등록 성공 시 처리할 로직
    ...
    replyWrap.addEventListener("click", (e) => {
        if (e.target && e.target.classList.contains("reply-delete")) {
            ...
            deleteReplyNotice(rno).then((response) => {
                // 삭제 성공 시 처리할 로직
    ...
    replyWrap.addEventListener("click", (e) => {
        ...
        `;
            ...
            confirmButton.addEventListener("click", () => {
                ...
                // 수정된 댓글을 서버로 보냄
                updateReplyNotice(list).then((response) => {
                    console.log(response);
                    // 댓글 목록 다시 로드
                    listReplyAxios(false, 1);
                });
